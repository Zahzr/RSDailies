...
export function makeCollapseButton(blockId, runtime = {}) {
  const btn = document.createElement('button');
  btn.type = 'button';
  btn.className = 'btn btn-secondary btn-sm mini-collapse-btn';

  const isCollapsedBlock = resolveCallback(runtime, 'isCollapsedBlock', () => false);
  const setCollapsedBlock = resolveCallback(runtime, 'setCollapsedBlock', () => {});
...
export function createHeaderRow(label, blockId, options = {}, runtime = {}) {
  const {
    className = '',
    rightText = '',
    onRightClick = null,
  ...
  const collapse = (collapsible && blockId) ? makeCollapseButton(blockId, runtime) : null;
